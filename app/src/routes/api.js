...
function registerRoutes(app, aiService, config, fishingData, subscriptionService, googlePlayBilling = null, weatherService = null, sessionAuth = null, authMiddleware = null, db = null) {
    ...
    app.get('/api/weather-debug', async (req, res) => {
        const location = req.query.location || 'Caesar Creek Lake, OH';
        const diag = { location, steps: [], timestamp: new Date().toISOString() };
        const { safeFetch } = require('../lib/safe-fetch');

        // Step 1: Test Nominatim directly
        try {
            const nomUrl = new URL('https://nominatim.openstreetmap.org/search');
            nomUrl.searchParams.set('q', location);
            nomUrl.searchParams.set('format', 'jsonv2');
            nomUrl.searchParams.set('limit', '1');
            const r = await safeFetch(nomUrl, { headers: { 'User-Agent': 'FishSmart-Pro/2.0' }, signal: AbortSignal.timeout(8000) });
            const d = await r.json();
            diag.steps.push({ step: 'nominatim', status: r.status, ok: r.ok, hasResult: Array.isArray(d) && d.length > 0, firstResult: d[0] ? { lat: d[0].lat, lon: d[0].lon } : null });
        } catch(e) { diag.steps.push({ step: 'nominatim', error: e.message }); }

        // Step 2: Test Open-Meteo geocoder
        try {
            const geoUrl = 'https://geocoding-api.open-meteo.com/v1/search?name=' + encodeURIComponent(location) + '&count=1&country=US&format=json';
            const r = await safeFetch(geoUrl, { signal: AbortSignal.timeout(8000) });
            const d = await r.json();
            diag.steps.push({ step: 'openmeteo-geo', status: r.status, ok: r.ok, hasResults: !!d?.results?.length, firstResult: d?.results?.[0] ? { lat: d.results[0].latitude, lon: d.results[0].longitude } : null });
        } catch(e) { diag.steps.push({ step: 'openmeteo-geo', error: e.message }); }

        // Step 3: Test Open-Meteo forecast with known coords
        try {
            const params = new URLSearchParams({ latitude: '39.5153', longitude: '-84.0005', current: 'temperature_2m,relative_humidity_2m,surface_pressure', temperature_unit: 'fahrenheit', timezone: 'auto' });
            const r = await safeFetch('https://api.open-meteo.com/v1/forecast?' + params, { signal: AbortSignal.timeout(5000) });
            const d = await r.json();
            diag.steps.push({ step: 'openmeteo-forecast', status: r.status, ok: r.ok, hasCurrent: !!d?.current, temp: d?.current?.temperature_2m });
        } catch(e) { diag.steps.push({ step: 'openmeteo-forecast', error: e.message }); }

        // Step 4: Check if OpenWeather key works
        if (config.openWeatherApiKey) {
            try {
                const r = await safeFetch('https://api.openweathermap.org/data/2.5/weather?q=' + encodeURIComponent(location) + '&appid=' + config.openWeatherApiKey + '&units=imperial', { signal: AbortSignal.timeout(5000) });
                diag.steps.push({ step: 'openweather', status: r.status, ok: r.ok });
            } catch(e) { diag.steps.push({ step: 'openweather', error: e.message }); }
        } else {
        ...
        if (weatherService) {
            try {
                const wx = await weatherService.getWeatherData(location);
                diag.steps.push({ step: 'getWeatherData', result: wx ? 'GOT DATA' : 'NULL', temp: wx?.main?.temp || wx?.temp });
            } catch(e) { diag.steps.push({ step: 'getWeatherData', error: e.message }); }
        } else {
            diag.steps.push({ step: 'getWeatherData', skipped: 'weatherService not configured' });
        }

        diag.env = { hasOpenWeatherKey: !!config.openWeatherApiKey, hasIpGeoKey: !!config.ipGeoApiKey, nodeVersion: process.version };
        res.json(diag);
    });
