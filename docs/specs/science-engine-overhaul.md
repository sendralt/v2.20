...
├── metabolic.js           → MODIFY: replace cubic decay with plateau model; re-evaluate T_opt; T_max = feeding cessation (not UILT)
...
- Metabolic (plateau model replacing cubic decay; T_opt re-evaluation per species; T_max = feeding cessation temperature, not UILT)
...
10. T_max = species-specific feeding cessation temperature (not UILT/CTMax); metabolic curve uses plateau model with broad peak zone, not cubic decay.
...
7. **Photoperiod integration:** **Supplement (layered approach).** Photoperiod is added as a bonus multiplier on top of the existing static time-of-day windows. The hardcoded windows (5-8, 17-20) remain as baseline; photoperiod adds seasonal correction (e.g., December dawn is later than June dawn). Lower risk than full replacement, keeps existing tests green.
 8. **T_max = feeding cessation temperature, NOT UILT/CTMax:** Scientific review incorrectly recommended using Upper Incipient Lethal Temperature (UILT) / Critical Thermal Maximum (CTMax, ~99°F for LMB). This is a **feeding prediction engine**, not a survival model. Fish cease active feeding at ~85-90°F, well below lethal limits. T_max must represent the **upper boundary of active feeding**, not death. Use species-specific feeding cessation temperatures from behavioral fisheries literature (e.g., Carlander 1977, state DNR feeding tables), not UILT/CTMax values from Beitinger et al. 2000.
 9. **Metabolic curve shape — plateau model, not cubic decay:** The current cubic decay function `(1-x)³` causes efficiency to crash catastrophically between T_opt and T_max (e.g., 78°F = 17% efficiency for LMB, despite being peak feeding temperature). Replace with a **plateau model**: efficiency stays at ~95-100% from T_opt to T_opt+8°F (broad peak zone), then decays gradually to T_max. This matches real fish thermal performance curves (Fry 1971, Brett 1971) and the project's own feeding activity descriptions.
 10. **T_opt re-evaluation per species:** Current T_opt values may be too low. For Largemouth Bass, T_opt=72°F produces a mismatch — 75-85°F is described as peak feeding in the project's own feeding table. Review fisheries literature for species-specific **preferred feeding temperatures** (not just lab metabolic optima). LMB likely needs T_opt closer to 78-80°F. All T_opt changes require benchmark runs per the boundaries section.
 11. **Water temperature model — USGS primary, hybrid is fallback only:** Scientific review incorrectly recommended replacing the 70/30 hybrid model with Mohseni et al. (1998) regression. The engine's `getLiveWaterTemp()` uses a clear priority chain: **(1) USGS live monitoring station data** is the primary method (actual measured water temp within 50mi), **(2) 70/30 hybrid thermal lag** is a last-resort fallback for edge cases only (no station found, invalid coords, USGS service error). No regression model can outperform real sensor data. The `source` field transparently flags `'usgs-live'` vs `'estimated'`. Mohseni regression is an **optional future improvement for the fallback path only** — not a top recommendation.
